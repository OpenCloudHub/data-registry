...
for dataset in fashion-mnist wine-quality emotion opencloudhub-readmes opencloudhub-readmes-rag-evaluation; do
  git tag -f "${dataset}-${VERSION}" -m "${dataset} ${VERSION}"
done

# Push all tags at once
git push -f origin fashion-mnist-${VERSION} wine-quality-${VERSION} emotion-${VERSION} opencloudhub-readmes-${VERSION} opencloudhub-readmes-rag-evaluation-${VERSION}
